...
exports.handler = async (lambdaEvent, context) => {
  ...
  const webhookSecret = process.env.STRIPE_WEBHOOK_SECRET2;
  ...
  try {
    ...
    if (stripeEvent.type === "checkout.session.completed") {
      ...
      const emailMessage = {
        ...
        text: `Bonjour ${metadata.customerName}, Voilà ! Votre réservation au Il Girasole est définitivement validée et nous vous en remercions. En cas d’annulation, vous pouvez nous contacter jusqu’à 48h avant votre réservation pour demander le remboursement de l'acompte. 
